...
export class LayoutComponent implements OnInit {
    ...
    confirmReq: ConfirmRequest | null = null;
    ...
    ngOnInit() {
        this.notifService.notifications$.subscribe(notifs => {
            this.mockNotifications = notifs;
            this.unreadNotifsCount = this.notifService.unreadCount;
        });
    ...
    get profilePicture(): string | null {
        ...
        return localStorage.getItem(`profile_picture_${this.user.email}`);
    }
    ...
    performSearch(): void {
        if (this.searchQuery) {
            ...
            this.searchResults = this.allMenuItems.filter(item => 
                ...
                (this.auth.isAdmin() || item.roles.some((r: string) => this.auth.hasRole(r) || r === 'ROLE_EMPLOYE'))
            );
